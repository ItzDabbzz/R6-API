import {
  ...
  R6Region,
  UbiAppId,
} from "../utilities/interfaces/enums";
...
import {
  R6LevelResponse,
  R6LifetimePlatform,
  R6LifetimeProfileData,
  R6LifetimeResponse,
  R6OperatorsPlatform,
  R6OperatorsProfileData,
  R6OperatorsResponse,
  R6SeasonResponse,
  R6UserResponse,
  UbiToken,
} from "../utilities/interfaces/http_interfaces";
import {
  AllSeasonCodes,
  CreateParameterString,
  GetNextRankRankPoints,
  NextRankFromRank,
  Percent,
  PlatformFamily,
  PreviousRankFromRank,
  RankFromInt,
  RankPointProgress,
  Ratio,
} from "../utilities/r6-utilities";
import { secondsSinceEpoch } from "../utilities/timestamps";
import { UbiAccountIds, UbiDataByUserId } from "../utilities/interfaces/simple";
import {
  DefaultR6LifetimesStats,
  DefaultR6OperatorsGamemode,
  MergeR6LifetimeStats,
  SimplifiedR6OperatorStats,
} from "../utilities/parsing-helpers";
...
export async function RequestFullProfile(
  ...
  if (!userData) {
    ...
    userData.forEach((user) => {
      ...
      switch (user.platform) {
        case R6RawPlatform.uplay:
          newPlatform = R6Platform.pc;
          break;
        case R6RawPlatform.psn:
          newPlatform = R6Platform.psn;
          break;
        case R6RawPlatform.xbl:
          newPlatform = R6Platform.xbox;
          break;
        default:
          return internalError;
      }
      ...
      operatorsPromises.push(
        RequestR6Operators(user.userId, user.profileId, newPlatform, tokenV2)
      );
      // Create request for this user's lifetime data.
      lifetimePromises.push(
        RequestR6Lifetime(user.userId, user.profileId, newPlatform, tokenV2)
      );
    });
  }
  ...
  levelData.forEach((level) => {
    if (level) {
      ...
      console.error(
        "No level data for profile. Continuing with default level."
      );
      // Cannot assign to levelDataMap without profileId
    }
  });
  ...
  operatorsData.forEach((operators) => {
    if (operators) {
      ...
      console.error(
        "No operator data for profile. Continuing with empty operator data."
      );
      // Using empty data instead of throwing an error
      // Cannot assign to operatorsDataMap without profileId
    }
  });
  ...
  lifetimeData.forEach((lifetime) => {
    if (lifetime) {
      ...
      console.error(
        "No lifetime data for profile. Continuing with default lifetime data."
      );
      // Cannot assign to lifetimeDataMap without profileId
    }
  });
  ...
  currentSeasonData.forEach((currentSeasonDataByPlatform) => {
    if (currentSeasonDataByPlatform) {
      ...
      currentSeasonDataMap = new Map([
        ...currentSeasonDataMap,
        ...currentSeasonDataByPlatform,
      ]);
    } else {
      console.error(
        "No current season data for profile. Continuing with available data."
      );
    }
  });
 ...
 */
async function RequestR6UserByUsername(
  usernames: string,
  platform: R6Platform,
  token: UbiToken
): Promise<R6User[] | void> {
  ...
  switch (platform) {
    case R6Platform.pc:
      rawPlatform = R6RawPlatform.uplay;
      break;
    case R6Platform.psn:
      rawPlatform = R6RawPlatform.psn;
      break;
    case R6Platform.xbox:
      rawPlatform = R6RawPlatform.xbl;
      break;
    default:
      throw Error("Bad platform specified.");
  }
 ...
 * @returns Array of R6Users.
 */
async function RequestR6UserById(
  profileId: string,
  token: UbiToken
): Promise<R6User[] | void> {
 ...
 */
async function RequestR6Level(
  userId: string,
  profileId: string,
  token: UbiToken
): Promise<R6Level | void> {
  const httpConfig = {
    method: "GET",
 ...
 */
async function RequestR6Operators(
  userId: string,
  profileId: string,
  platform: R6Platform,
  token: UbiToken
): Promise<R6Operators | void> {
  ...
  const httpConfig = {
    method: "GET",
    url: `https://prod.datadev.ubisoft.com/v1/users/${userId}/playerstats?${CreateParameterString(
      parameters
    )}`,
    headers: {
      Authorization: `ubi_v1 t=${token.ticket}`,
  ...
  try {
    ...
    const profileData = new Map<string, R6OperatorsProfileData>(
      Object.entries(data.profileData)
    );
    ...
    profileData.forEach((profileData) => {
      ...
      switch (newPlatform) {
        case "PC":
          dataByPlatform = profileData.platforms?.PC;
          break;
        case "PLAYSTATION":
          dataByPlatform = profileData.platforms?.PS4;
          break;
        case "XONE":
          dataByPlatform = profileData.platforms?.XONE;
          break;
        default:
          return;
      }

      if (!dataByPlatform || !dataByPlatform.gameModes) {
        return;
      } else {
        ...
        if (gameModes.all) {
          ...
          if (all.teamRoles) {
            ...
            attackers?.forEach((operatorData) => {
              parsed.overall.attackers![
                operatorData.statsDetail.toLowerCase()
              ] = SimplifiedR6OperatorStats(operatorData);
            });

            defenders?.forEach((operatorData) => {
              parsed.overall.defenders![
                operatorData.statsDetail.toLowerCase()
              ] = SimplifiedR6OperatorStats(operatorData);
            });
          }
        ...
        if (gameModes.casual) {
          ...
          if (casual.teamRoles) {
            let attackers =
              casual.teamRoles.attacker || casual.teamRoles.Attacker;
            let defenders =
              casual.teamRoles.defender || casual.teamRoles.Defender;

            attackers?.forEach((operatorData) => {
              parsed.casual.attackers![operatorData.statsDetail.toLowerCase()] =
                SimplifiedR6OperatorStats(operatorData);
            });

            defenders?.forEach((operatorData) => {
              parsed.casual.defenders![operatorData.statsDetail.toLowerCase()] =
                SimplifiedR6OperatorStats(operatorData);
            });
          }
        }
        ...
        if (gameModes.ranked) {
          ...
          if (ranked.teamRoles) {
            let attackers =
              ranked.teamRoles.attacker || ranked.teamRoles.Attacker;
            let defenders =
              ranked.teamRoles.defender || ranked.teamRoles.Defender;

            attackers?.forEach((operatorData) => {
              parsed.ranked.attackers![operatorData.statsDetail.toLowerCase()] =
                SimplifiedR6OperatorStats(operatorData);
            });

            defenders?.forEach((operatorData) => {
              parsed.ranked.defenders![operatorData.statsDetail.toLowerCase()] =
                SimplifiedR6OperatorStats(operatorData);
            });
          }
        }
        ...
        if (gameModes.unranked) {
          ...
          if (unranked.teamRoles) {
            let attackers =
              unranked.teamRoles.attacker || unranked.teamRoles.Attacker;
            let defenders =
              unranked.teamRoles.defender || unranked.teamRoles.Defender;

            attackers?.forEach((operatorData) => {
              parsed.unranked.attackers![
                operatorData.statsDetail.toLowerCase()
              ] = SimplifiedR6OperatorStats(operatorData);
            });

            defenders?.forEach((operatorData) => {
              parsed.unranked.defenders![
                operatorData.statsDetail.toLowerCase()
              ] = SimplifiedR6OperatorStats(operatorData);
            });
          }
 ...
 */
async function RequestR6Lifetime(
  userId: string,
  profileId: string,
  platform: R6Platform,
  token: UbiToken
): Promise<R6Lifetime | void> {
  let newPlatform: string;
  ...
  try {
    ...
    const profileData = new Map<string, R6LifetimeProfileData>(
      Object.entries(data.profileData)
    );
    ...
    profileData.forEach((profileData) => {
      ...
      if (!dataByPlatform || !dataByPlatform.gameModes) {
        ...
        if (gameModes) {
          ...
          parsed.ranked = MergeR6LifetimeStats(
            parsed.ranked,
            gameModes.unranked
          );
          parsed.unranked = MergeR6LifetimeStats(
            parsed.ranked,
            gameModes.unranked
          );
        }
 ...
 */
async function RequestR6Season(
  userDataByUserId: Map<string, UbiDataByUserId>,
  token: UbiToken
): Promise<Map<string, R6Season> | void> {
  const parameters = {
  ...
  const httpConfig = {
    method: "GET",
    url: `https://public-ubiservices.ubi.com/v2/spaces/0d2ae42d-4c27-4cb7-af6c-2099062302bb/title/r6s/skill/full_profiles?${CreateParameterString(
      parameters
    )}`,
    headers: {
      Authorization: `ubi_v1 t=${token.ticket}`,
                ...
                if (platformFamily === desiredPlatformFamily) {
                  ...
                  if (gameMode === "ranked") {
                    ranked = {
                      ...
                      kdRatio: Ratio(
                        currentSeasonData.kills,
                        currentSeasonData.deaths
                      ),
                      ...
                      winPercent: Percent(
                        currentSeasonData.match_outcomes.wins,
                        currentSeasonData.match_outcomes.losses
                      ),
